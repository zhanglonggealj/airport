...
//			LoggerUtils.error(StringUtils.class, "URLCode转换为字符串失败;value:" + value,e);
	...
	public static String getSortAnswerByAnswer(String questionAnswer) {
		...
		questionAnswer = questionAnswer.replaceAll(" ","");
		char[] array = questionAnswer.toCharArray();
		if (array.length <= 0) {
			return "";
		}
		Arrays.sort(array);
		String newAnswer = new String(array);

		// 加逗号
		StringBuilder tmp = new StringBuilder();
		for(int i=0;i<newAnswer.length();i++){
			tmp.append(newAnswer.substring(i, i+1));
	...
	public static void main(String[] args) {
		System.out.println(getSortAnswerByAnswer("ABC"));
	}
